...
function completedAtMs(job) {
  ...
  const iso = job.JobCompleteTime || job.completedAt_ISO || job.completedAt;
...
function buildCompletedJobRecord(job, opts) {
  ...
  const record = {
    ...
    source: str(opts.source || job.source || 'dispatch_complete'),
  };

  if (job.CompletedBy) record.CompletedBy = str(job.CompletedBy);
  if (job.Account_id || job.AccountId || job.jobAccountId) {
    record.accountId = str(job.Account_id || job.AccountId || job.jobAccountId);
...
async function upsertCompletedJobFromDispatch(job, deps, opts) {
  ...
  let closedPath = null;
  if (opts.writeClosedJobs !== false && typeof deps.push === 'function') {
    // Only push closedJobs when no completedJobs existed (avoid duplicate closed rows on enrich)
