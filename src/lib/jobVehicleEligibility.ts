...
export function driverEligibleForJob(
  job: Pick<Job, 'vehicleType' | 'passengers'>,
  driver: Driver,
): boolean {
  return driverMeetsJobVehicleType(job, driver) && driverMeetsJobPassengers(job, driver);
}
...
export function filterDriversForRequirements(
  drivers: Driver[],
  req: { vehicleType?: string; passengers?: number },
): Driver[] {
  const job = {
