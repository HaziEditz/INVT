...
### 30-second no-response timeout + Automatic display + No One badge (session 20)
95. **30-second driver no-response timeout was never firing** — `resolveAfter2Secondsx`/`resolveAfter2Seconds` use a Firebase `on("value")` listener that returns early with `if ($respp['status'] === 'Sent') { return; }` when the pre-seeded `{jobstatus:'Offer',status:'Sent'}` value is present. Since the driver never changes that value (in no-response case), `countr` never increments → the `countr >= 6` timeout branch never fires → the 30-second cutoff was permanently dead. Fixed: added a hard `setTimeout(30000)` in both `acknowledgemethodx` and `acknowledgemethod` that calls the new `_handleDriverNoResponse(vehicle, driverid, bookid)` function. This function: (a) checks the new `_jobHandledFlags[bookid]` guard to skip if the driver already accepted/rejected; (b) removes the Firebase `joback` and `notification` nodes; (c) tries `online/1216/{driverid}/vehiclestatus = "Away"` best-effort; (d) POSTs `[DriverTimedOut]` to the server. Flag `_jobHandledFlags[bookid] = true` is also set at the top of each accept/reject branch in both listener functions, and a guard `if (_jobHandledFlags[bookid]) { refaz.off(); return; }` is added at the top of each Firebase listener callback.
96. **`[DriverTimedOut]` server action** — New handler in both DataProcessor and DataSelectorLess paths. Finds the job by bookingId, reads `job.initialDispatchMode` (new field added to `InsertBookingv4`: `'Auto'` for driverId≤0, `'NoOne'` for driverId===-1, `'Offered'` for driverId>0), and restores: Auto → `BookingStatus='Pending'`, `DriverId=-2`; NoOne → `BookingStatus='No One'`, `DriverId=-1`. Clears `VehicleId`, `VehicleNo`, `CallSign`, `drivername`, `UserFName`, `UserLName`. Finds the driver in `ZONE_DRIVERS`, sets `vehiclestatus='Away'`, clears job fields, and bumps `zonequeue` to `max(same-vehicletype)+1` (puts driver last in queue for their vehicle type).
97. **"Pending" status displayed as "Automatic" in badges** — All `{{value.BookingStatus}}` expressions in the Unassigned-tab red badge now use `{{value.BookingStatus=='Pending'?'Automatic':value.BookingStatus}}` (replace_all across all 4 Unassigned/Delivery/OfferUnassigned views). The separate purple "Automatic" badge (`ng-if="value.BookingStatus=='Pending'"`) was already present and correct.
98. **"No One" badge added** — A red `<i class="fa fa-ban"></i> No One` badge (`ng-if="value.BookingStatus=='No One'"`) was added after the existing "Automatic" badge in all 3 Unassigned tab views, so dispatchers can immediately see which jobs are in "No One" auto-dispatch mode.
