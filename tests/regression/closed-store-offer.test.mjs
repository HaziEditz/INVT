...
test('booking id reuse: stale closedJobStore must not purge live Offered on auto-dispatch reconcile', async () => {
  ...
  const pending = await h.poll(
    reusedId,
    (t) =>
      t.jobStore?.found === true &&
      String(t.jobStore?.lifecycle?.BookingStatus || '') === 'Pending' &&
      t.closedStoreDiagnosis?.wouldBlockUpdateBooking === false &&
      t.closedStoreDiagnosis?.updateBookingBug === false,
    { timeoutMs: 60000 },
  );
  assert.ok(
    (pending.closedStoreDiagnosis?.staleClosedEntryCount ?? 0) >= 1,
    'closedJobStore history may remain on id reuse — edits must still be allowed',
  );
  ...
  assert.equal(offered.closedStoreDiagnosis?.wouldBlockUpdateBooking, false);
