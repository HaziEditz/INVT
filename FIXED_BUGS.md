...
| DISPATCH-LIST-REFRESH-LAG | Dispatch list ~2 steps behind driver: (1) queue accept stuck in **UA** until Arrived; (2) completed job ghost on **Active** until manual refresh. Same gap: indefinite `shouldPreserveAbsentStoreJob` + weak refresh + `pendingjobs` delete on queue/promote + early blacklist clear. Follow-up: Queue tab blank while Queued — `purgeStalePendingForQueuedBookings` dropped the Queued `pendingjobs` mirror when store was already Queued before `bookingsRef` caught up. Follow-up2: Queue sticky through Assign/Active — `liveQueued` blocked pending promotions + queue-await forced Queued over Assigned/Active + mergeJobStatus seq-pinned Queued. | INVT: tighten preserve; authoritative refresh; hold completed blacklist; `pendingjobs` Queued/Assigned mirrors; purge keeps Queued mirrors until bookings confirms then drops on forward lifecycle; allow Queued→Assigned+ without seq bump; clear queue-await on forward. Guard: `tests/regression/dispatch-list-refresh-lag.test.mjs` |
| AUTO-DISPATCH-EMPTY-VTYPE | Auto-dispatch / assign silently rejected Available drivers when `vehicletype` was missing/empty while the job required Sedan/Van/WAV — `_driverEligibleForJob` returned false with no skip reason. | INVT: empty vehicletype treated as eligible (warn log); guard: `tests/regression/vehicle-eligibility.test.mjs` (`empty vehicletype allows Sedan`) |
| PAX-WAITING-ORPHAN-CANCEL | Passenger-app ASAP (`Status:Waiting`, `PickupAddress` schema) auto-cancelled by Watchdog-Pax with only lowercase `status:Cancelled` on allbookings — pendingjobs stayed Waiting; job never entered jobStore so dispatch Cancel was a silent no-op. | INVT: map passenger fields in `_fbRecToJob`; orphan Firebase cancel when not in jobStore; prefer terminal Status; Watchdog-Pax stamps all Status fields + clears pendingjobs. Guard: `tests/regression/passenger-waiting-orphan-cancel.test.mjs` |
...
| ID | Issue | Notes |
|----|-------|-------|
| DISPATCH-ACTIVE-REFRESH-LAG | *(Superseded by **DISPATCH-LIST-REFRESH-LAG** above.)* Completed job stuck on Active until manual refresh — same root cause as queue UA lag. | Fixed: see `DISPATCH-LIST-REFRESH-LAG` + `tests/regression/dispatch-list-refresh-lag.test.mjs`. |
