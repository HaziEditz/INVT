...
| DISPATCH-LIST-REFRESH-LAG | Dispatch list ~2 steps behind driver: (1) queue accept stuck in **UA** until Arrived; (2) completed job ghost on **Active** until manual refresh. Same gap: indefinite `shouldPreserveAbsentStoreJob` + weak refresh + `pendingjobs` delete on queue/promote + early blacklist clear. | INVT: tighten preserve to optimistic window only; authoritative `dispatchConsole/refresh` for assign/accept/active; hold completed blacklist through suppress; `pendingjobs` Queued/Assigned mirrors. Guard: `tests/regression/dispatch-list-refresh-lag.test.mjs` |
