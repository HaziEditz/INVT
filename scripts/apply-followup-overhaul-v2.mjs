...
const alarmBodyOld = '<div class="modal-body modal-alarm-box" style="height:300px; overflow-y:auto; padding:12px 16px; background:#f9fafb;">\n                </div>';
const alarmBodyNew = `<div class="modal-body" style="height:300px; overflow-y:auto; padding:12px 16px; background:#f9fafb;">
...
if (html.includes(alarmBodyOld)) {
  html = html.replace(alarmBodyOld, alarmBodyNew);
  console.log('OK: Alarms ng-repeat template');
} else if (!html.includes('bwAlarmList')) {
  console.warn('SKIP: Alarms modal body not found');
}

// Alarms() — date-aware datetime + scope list
html = html.replace(
...
// cancelactivejob → unified cancelBooking API
html = html.replace(
  /\$scope\.cancelactivejob = function\(id\)\{[\s\S]*?\n                    \}/,
...
console.log('Follow-up v2 patch applied to Default.aspx');
